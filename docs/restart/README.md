...
| `75-aquarius-keys.sh` | Mac-style keyboard shortcuts, on by default — Copy is Command-C. Installs what the two steps above built, and checks the whole feature. Since 2026-09-16 that includes both **graphical switches** for the Mac-or-Windows choice: our GNOME toggle in the quick settings menu and the KDE System Settings page. Plain-language guide: [`aquarius-keys.md`](aquarius-keys.md). |
