...
| `71-capture.sh` | **Screenshots and screen recording** (Royce, 2026-09-13). Puts `grim`, `slurp`, `wl-clipboard` and `wf-recorder` on the image and reads back our own `/usr/libexec/aquarius-capture` — the one program behind both top-bar buttons and all six keyboard shortcuts. Everything lands in `~/Screenshots`, which is pinned in the Files sidebar, and a screenshot also goes on the clipboard. ⚠️ `grim` had been in this repository since the restart and was still missing from the finished image: it lived only in the labwc *builder* stage, which is thrown away. ⚠️ And "pick a window" honestly means "drag a box round it" — the Wayland standard that lists windows carries no position or size, and labwc has no private channel that does. See [`screenshots.md`](screenshots.md). |
