...
| | |
| --- | --- |
| **Step 1 of 3** | *How should keyboard shortcuts work?* Mac, or Windows. Mac is already chosen. Mac means ⌘C, Windows means Ctrl+C. It is the keyboard only — nothing on the screen moves. (Until 17 September 2026 this answer also moved the window buttons to the left. That was dropped: applications which draw their own title bar never followed it, so the buttons are now always on the right, for everybody.) |
