...
PlasmaCore.ToolTipArea {
    ...
    // AQUARIUS DEVIATION — the "smart launcher" is not available to us.
    //
    // Upstream builds a helper here that listens for two extras an app can
    // broadcast about itself:
    //
    //   a COUNT   the little number badge on an icon — 3 unread mails, and so on
    //   a PROGRESS BAR   a bar across the icon while a copy or download runs
    //
    // Both come from a C++ class called SmartLauncherItem, which lives inside
    // Plasma's compiled copy of this widget. Our dock is a folder of QML files
    // (the reasons are in AquariusBackend.qml and FORK-NOTES.md), so that class
    // cannot be reached, and QML has no way to build one.
    //
    // Upstream's line was:
    //     const component = Qt.createComponent(
    //         "plasma.applet.org.kde.plasma.taskmanager", "SmartLauncherItem");
    //
    // Leaving it in would not fail loudly — it would throw inside a signal
    // handler every time an icon appeared, and fill the log while the dock kept
    // going. So it is gone, and `smartLauncherItem` simply stays null.
    //
    // Nothing else needed changing: every place that draws a badge or a progress
    // bar already checks `task.smartLauncherItem` first, so those two overlays
    // are never asked for. TaskBadgeOverlay.qml and TaskProgressOverlay.qml are
    // still carried here, byte-identical to upstream, so that restoring this
    // means restoring one function rather than three files.
    //
    // COST: no unread-count badges, and no progress bars on dock icons. Both are
    // opt-in features an app has to broadcast; most do not.
    ...
    TapHandler {
        ...
        onTapped: (eventPoint, button) => {
            if (button === Qt.MiddleButton) {
                if (Plasmoid.configuration.middleClickAction === AquariusBackend.NewInstance) {
                    tasksModel.requestNewInstance(modelIndex());
                } else if (Plasmoid.configuration.middleClickAction === AquariusBackend.Close) {
                    tasksModel.requestClose(modelIndex());
                } else if (Plasmoid.configuration.middleClickAction === AquariusBackend.ToggleMinimized) {
                    tasksModel.requestToggleMinimized(modelIndex());
                } else if (Plasmoid.configuration.middleClickAction === AquariusBackend.ToggleGrouping) {
                    tasksModel.requestToggleGrouping(modelIndex());
                } else if (Plasmoid.configuration.middleClickAction === AquariusBackend.BringToCurrentDesktop) {
